...
func _execute_attack() -> void:
	if target and global_position.distance_to(target.global_position) <= attack_range + 20.0:
		var health := target.get_node_or_null("HealthComponent") as HealthComponent
		var prot := target.get_node_or_null("ProtectionComponent") as ProtectionComponent
		var dmg := damage_hearts
		if prot: dmg = prot.handle_incoming_damage(dmg)
		if health: health.take_damage(dmg)

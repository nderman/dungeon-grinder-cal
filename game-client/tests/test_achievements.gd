...
func run() -> void:
	...
	# Michael Bay (the AoE-spam fix): tier 0 + a long personal cooldown. On floors 1-3 it still pays
	# a (Bronze) Weapon Box; on floors 4+ it only heckles; and its cooldown is its own 45s, not the 12s default.
	...
	AchievementManager._repeat_cd.erase("michael_bay")
	SignalBus.ratings_spike.emit("BOOM")
	check(titles.size() == 1 and not ("Box" in String(titles[-1])), "tier-0 BOOM only heckles past floor 3 — no box")
